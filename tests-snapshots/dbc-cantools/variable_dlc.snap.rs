...
#[allow(dead_code)]
pub const DBC_FILE_NAME: &str = "variable_dlc";
...
impl Messages {
    ...
    pub fn from_can_message(id: Id, payload: &[u8]) -> Result<Self, CanError> {
        let res = match id {
            TestMessage1::MESSAGE_ID => {
                Messages::TestMessage1(TestMessage1::try_from(payload)?)
            }
            TestMessage2::MESSAGE_ID => {
                Messages::TestMessage2(TestMessage2::try_from(payload)?)
            }
    ...
    pub fn set_signal1(&mut self, value: u8) -> Result<(), CanError> {
        if value < 0_u8 || 0_u8 < value {
            return Err(CanError::ParameterOutOfRange {
                message_id: TestMessage1::MESSAGE_ID,
            });
        }
        self.raw.view_bits_mut::<Lsb0>()[0..8].store_le(value);
        Ok(())
...
impl TestMessage2 {
    ...
    pub fn set_signal4(&mut self, value: u16) -> Result<(), CanError> {
        if value < 0_u16 || 0_u16 < value {
            return Err(CanError::ParameterOutOfRange {
                message_id: TestMessage2::MESSAGE_ID,
            });
        }
        self.raw.view_bits_mut::<Lsb0>()[0..15].store_le(value);
        Ok(())
